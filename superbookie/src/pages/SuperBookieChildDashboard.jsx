...
const SuperBookieChildDashboard = () => {
    const { superBookieId } = useParams();
    ...
    const loadPlayers = useCallback(async () => {
        if (!superBookieId || playerList.length > 0) return;
        setPlayersLoading(true);
        try {
            const res = await fetchWithAuth(
                `${API_BASE_URL}/bookie/super-bookies/${superBookieId}/players`,
            );
    ...
    }, [superBookieId, playerList.length]);
    ...
    const sbName = data?.superBookie?.username || PANEL_LABEL;

    return (
        <Layout title={`${PANEL_LABEL} dashboard`}>
            <div className="min-w-0 max-w-6xl mx-auto space-y-4">
                <div>
                    <Link
                        to="/super-bookies"
                        className="text-gray-500 hover:text-emerald-600 text-sm inline-flex items-center gap-1 mb-2"
                    >
                        <FaArrowLeft className="w-3 h-3" />
                        Back to {PANEL_LABEL} list
                    </Link>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                            <h1 className="text-2xl font-bold text-[#1B3150]">{sbName}</h1>
                            <p className="text-sm text-gray-500">
                                {PANEL_LABEL} · {Number(data?.superBookie?.commissionPercentage ?? 0)}% commission
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <>
                        ...
                        {!activeTab && (
                            <p className="text-sm text-gray-500 text-center py-8 rounded-xl border border-dashed border-gray-200 bg-gray-50/80">
                                Select Players or Commission to view details.
                            </p>
                        )}
...
export default SuperBookieChildDashboard;
