...
const HalfSangamBid = ({ title, gameType, betType, embedInSingleScroll = false }) => {
    ...
    const inputCl = 'flex-1 min-w-0 bg-gray-100 border border-gray-200 text-gray-800 placeholder-gray-400 rounded-full py-2.5 min-h-[40px] px-4 text-center text-sm focus:ring-2 focus:outline-none focus:ring-sb-primary focus:border-sb-primary';
            ...
            <div className="px-3 sm:px-4 py-4 md:max-w-4xl md:mx-auto">
                ...
                <div className="md:grid md:grid-cols-2 md:gap-8 md:items-start space-y-6 md:space-y-0">
                    ...
                    <div className="mt-6 md:mt-0">
                        <div className="grid grid-cols-[1.4fr_0.7fr_0.6fr] gap-2 text-center text-sb-primary font-bold text-xs mb-2 px-2">
                            <div className="truncate">Sangam</div>
                            <div className="truncate">Amount</div>
                            <div className="truncate">Delete</div>
                        </div>
                        <div className="h-px bg-sb-primary/20 w-full mb-2" />
                        {bids.length === 0 ? null : (
                            <div className="space-y-2">
                                {bids.map((b) => (
                                    <div
                                        key={b.id}
                                        className="grid grid-cols-[1.4fr_0.7fr_0.6fr] gap-2 text-center items-center py-2.5 px-3 bg-sb-primary/5 rounded-lg border border-sb-primary/20 text-sm"
                                    >
...
function HalfSangamFormSection({ flipped, setFlipped, bids, setBids, session, showWarning, inputCl, labelCl, sanitizeDigits, sanitizePoints, isValidAnyPana }) {
    ...
    return (
        <div className="p-4 space-y-3">
            <h3 className="text-sb-primary font-semibold text-sm">{titleText}</h3>
            ...
            <button type="button" onClick={handleAdd} className="w-full bg-sb-primary text-white font-bold py-3 min-h-[44px] rounded-lg shadow-md hover:bg-sb-primary-dark transition-all active:scale-[0.98]">
                Add to List
            </button>
        </div>
    );
