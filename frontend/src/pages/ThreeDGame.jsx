...
const ThreeDGame = () => {
  ...
  const [insufficientBalancePopupMsg, setInsufficientBalancePopupMsg] = useState('');
  ...
  const historyTicketsForModal = useMemo(() => {
    ...
    [...localList, ...backendList].forEach((ticket) => {
      ...
      const slotFallback = String(ticket?.slotStartIso || '').trim();
      const key = outcome === 'cancelled'
        ? `cancelled:${tid || String(ticket?.id || ticket?.gameId || '')}`
        : (tid ? `buy:${tid}` : `slot:${slotFallback || String(ticket?.id || ticket?.gameId || '')}`);
  ...
  const visibleBetCards = useMemo(
    () => (Array.isArray(bets) ? [...bets].reverse().slice(0, MAX_VISIBLE_BET_CARDS) : []),
    [bets],
  );
  ...
  const loadBackendHistoryTickets = useCallback(async (opts = {}) => {
    ...
    try {
      ...
      const mapped = rows.map((row) => {
        ...
        const pendingCount = Number(row?.pendingBets || 0);
        return {
          ...
          totalPoints: Number(row?.totalStake || 0),
          totalWin: Number(row?.totalWinPayout || 0),
          outcome: String(row?.outcome || 'pending').toLowerCase(),
          isAdvanceDraw: Boolean(row?.isAdvanceDraw),
  ...
  }, [normalizeNumberForMode, selectedPanels]);
  ...
  const handleBuy = useCallback(async () => {
    const fail = (msg) => {
      ...
      if (String(msg || '').toLowerCase().includes('insufficient balance')) {
        setInsufficientBalancePopupMsg(String(msg));
      }
    ...
    /** One entry per `targetSlots` index — API `ticketId` must match history merge keys (avoids duplicate GM… rows). */
    const perSlotSaveMeta = [];
    try {
      for (const slotStartIso of targetSlots) {
        ...
        perSlotSaveMeta.push({
          ticketId: String(saved?.data?.ticketId || '').trim() || null,
          slotStartIso: String(saved?.data?.slotStartIso || '').trim() || String(slotStartIso || '').trim(),
        });
      }
    } catch (e) {
    ...
    const resolvedQuizId = Number.isInteger(Number(selectedQuizId)) && Number(selectedQuizId) > 0
      ? Number(selectedQuizId)
      : null;
    const displayUser = String(playerIdentity || '').trim() || 'user';
    const createdTickets = targetSlots.map((requestedSlotIso, idx) => {
      const meta = perSlotSaveMeta[idx];
      const slotStartIso = meta?.slotStartIso || String(requestedSlotIso || '').trim();
      const ticketId = meta?.ticketId || null;
      ...
      const fallbackGameId = generateGameId();
      return {
        id: ticketId ? `backend-ticket-${ticketId}` : Date.now() + idx,
        ticketId,
        ...
        gameId: ticketId || fallbackGameId,
    ...
    setLastTxnId(createdTickets[0]?.ticketId || createdTickets[0]?.gameId || '');
  ...
  const handleCancelPendingTicket = useCallback(async () => {
    ...
    try {
      ...
      pendingRows.forEach((row) => {
        ...
        const ticketId = String(row?.ticketId || '').trim();
        const key = ticketId || `slot:${slotStartIso}`;
        if (!byTicket.has(key)) {
          byTicket.set(key, {
            key,
            ticketId: ticketId || null,
        ...
        const slotStartMs = new Date(slotStartIso).getTime();
        if (Number.isFinite(createdAtMs) && Number.isFinite(slotStartMs) && (slotStartMs - createdAtMs) > ((GAME_INTERVAL_SECONDS * 1000) + (60 * 1000))) {
          bucket.isAdvanceDraw = true;
        }
      });
      options.push(...[...byTicket.values()]
        .sort((a, b) => b.latestCreatedAtMs - a.latestCreatedAtMs)
        .map((item) => ({
          ...item,
          drawLabel: formatDrawEndLabelFromSlotStartIso(item.slotStartIso),
          ticketLabel: item.ticketId || item.slotStartIso,
        })));
    } catch (_) {
      // If backend lookup fails, fallback below.
    }

    if (!options.length) {
      const fallback = (historyTicketsForModal || []).filter((ticket) => String(ticket?.outcome || '').toLowerCase() === 'pending');
      fallback.forEach((ticket) => {
        const slotStartIso = String(ticket?.slotStartIso || '').trim();
        if (!slotStartIso) return;
        options.push({
          key: String(ticket?.ticketId || '').trim() || `slot:${slotStartIso}`,
          ticketId: String(ticket?.ticketId || '').trim() || null,
          slotStartIso,
          totalPoints: Number(ticket?.totalPoints || 0),
          latestCreatedAtMs: new Date(ticket?.createdAt || 0).getTime(),
          isAdvanceDraw: Boolean(ticket?.isAdvanceDraw),
          drawLabel: ticket?.drawTime || formatDrawEndLabelFromSlotStartIso(slotStartIso),
          ticketLabel: String(ticket?.ticketId || ticket?.gameId || slotStartIso),
        });
      });
    }
    ...
    setCancelBetDialog({
      ...
      description: 'Choose a bet and tap Cancel.',
      points: Number(selected?.totalPoints || 0),
      options,
      selectedOptionKey: selected?.key || '',
    });
  }, [bets, formatDrawEndLabelFromSlotStartIso, historyTicketsForModal]);
  ...
  const handleConfirmCancelBet = useCallback(async () => {
    ...
    const nowMs = Date.now();
    const selectedOption = cancelBetDialog?.type === 'ticket-picker'
      ? (Array.isArray(cancelBetDialog?.options)
        ? cancelBetDialog.options.find((opt) => String(opt?.key || '') === String(cancelBetDialog?.selectedOptionKey || ''))
        : null)
      : null;
    if (cancelBetDialog?.type === 'ticket-picker' && !selectedOption) {
      setValidationMsg('Please select a ticket to cancel.');
      setIsCancelSubmitting(false);
      return;
    }

    /** After "Cancel This Bet", dialog is `type: 'ticket'` — must use `ticketBackendId`, not picker `selectedOption` (would be null and cancel entire slot). */
    const backendTicketForCancel = cancelBetDialog?.type === 'ticket'
      ? String(cancelBetDialog?.ticketBackendId || '').trim()
      : '';
    const targetSlotStartIso = String(
      selectedOption?.slotStartIso
        || (cancelBetDialog?.type === 'ticket' ? cancelBetDialog?.slotStartIso : '')
        || cancelBetDialog?.slotStartIso || '',
    ).trim();
    const optTicketId = backendTicketForCancel || String(selectedOption?.ticketId || '').trim();
    const cancellable = (historyTicketsForModal || []).find((ticket) => {
      if (optTicketId) {
        return String(ticket?.ticketId || '').trim() === optTicketId;
      }
    ...
    const refundAmount = Number(selectedOption?.totalPoints || cancellable?.totalPoints || cancelBetDialog?.points || 0);
    const ticketLabel = selectedOption?.ticketLabel || cancellable?.gameId || cancelBetDialog?.ticketLabel || String(cancellable?.id || '');

    if (!cancellable && !selectedOption && cancelBetDialog?.type !== 'ticket') {
    ...
    try {
      ...
      if (slotStartIso) {
        const selectedTid = optTicketId;
        if (selectedTid) {
          const cancelled = await cancelMyQuizTicket(selectedTid, '3d');
          ...
          for (const row of cancellableRows) {
            // eslint-disable-next-line no-await-in-loop
            const cancelledRow = await cancelMyQuizBet(row.id, '3d');
            const b = Number(cancelledRow?.data?.balance);
            if (Number.isFinite(b)) backendBalance = b;
          }
        }
      }
    ...
    setTicketHistory((prev) => prev.map((ticket) => {
      const ticketTid = String(ticket?.ticketId || '').trim();
      if (optTicketId) {
        if (ticketTid !== optTicketId) return ticket;
      } else if (targetSlotStartIso) {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-gradient-to-br from-[#0b1223] via-[#182a4a] to-[#1e3a5f]">
      ...
      >
        ...
        {insufficientBalancePopupMsg ? (
          <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/55 p-4">
            <div className="w-full max-w-md rounded-2xl border border-rose-300 bg-white p-5 shadow-[0_18px_40px_rgba(15,23,42,0.4)]">
              <h3 className="text-xl font-extrabold text-rose-700">Insufficient Balance</h3>
              <p className="mt-2 text-base font-semibold text-slate-700">{insufficientBalancePopupMsg}</p>
              <div className="mt-4 flex justify-end">
                <button
                  type="button"
                  onClick={() => setInsufficientBalancePopupMsg('')}
                  className="rounded-lg border border-rose-300 bg-rose-600 px-4 py-2 text-sm font-bold text-white hover:bg-rose-700"
                >
                  OK
                </button>
              </div>
        ...
        {cancelBetDialog ? (
          <div
            className="fixed inset-0 z-[87] flex items-center justify-center bg-black/80 p-1 backdrop-blur-[2px] sm:p-4"
            onMouseDown={(e) => {
              if (isCancelSubmitting || e.target !== e.currentTarget) return;
              setCancelBetDialog(null);
            }}
            role="dialog"
            aria-modal="true"
            aria-labelledby="cancel-bet-dialog-title"
          >
            <div
              className={`w-[calc(100vw-8px)] max-w-[96vw] rounded-2xl border border-[#475569] bg-gradient-to-b from-[#0f172a] to-[#111827] p-3 text-white shadow-2xl sm:max-w-3xl sm:p-7 ${isCancelSubmitting ? 'animate-pulse' : ''}`}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2">
                  <CircleX className="h-8 w-8 shrink-0 text-rose-400" />
                  <h4 id="cancel-bet-dialog-title" className="text-[clamp(2rem,8vw,2.5rem)] font-black leading-tight">{cancelBetDialog.title || 'Cancel Bet'}</h4>
                </div>
                <button
                  type="button"
                  aria-label="Close dialog"
                  disabled={isCancelSubmitting}
                  className="shrink-0 rounded-lg border border-[#475569] bg-[#1e293b] p-2 text-white hover:bg-[#334155] disabled:opacity-50"
                  onClick={() => setCancelBetDialog(null)}
                >
                  <X className="h-6 w-6" aria-hidden />
                </button>
              </div>
              <p className="mt-2 text-[clamp(1.28rem,5vw,1.55rem)] font-semibold text-[#dbe5f3]">{cancelBetDialog.description}</p>
              {cancelBetDialog.type !== 'ticket-picker' ? (
                <p className="mt-2 text-[clamp(1.25rem,5.2vw,1.45rem)] font-bold text-amber-300">
                  Amount: ₹{Number(cancelBetDialog.points || 0)}
                </p>
              ) : null}
              {cancelBetDialog.type === 'ticket' || cancelBetDialog.type === 'ticket-picker' ? (
              ...
              {cancelBetDialog.type === 'ticket-picker' ? (
                <div className="mt-4 max-h-[22rem] overflow-y-auto rounded-xl border border-[#334155] bg-[#0b1220] p-2.5 sm:p-3">
                  {(Array.isArray(cancelBetDialog.options) ? cancelBetDialog.options : []).map((option) => {
                    const singleOptionMode = (Array.isArray(cancelBetDialog.options) ? cancelBetDialog.options.length : 0) === 1;
                    return (
                      <div
                        key={option.key}
                        className="mb-3 rounded-xl border border-[#3f4d63] bg-[#111827] px-3.5 py-3.5 text-[#cbd5e1] last:mb-0 sm:px-4"
                      >
                        <div className="break-all text-[18px] font-extrabold leading-tight text-white sm:text-[18px]">
                          {option.ticketId ? `Ticket: ${option.ticketId}` : `Slot: ${option.slotStartIso}`}
                        </div>
                        <div className="mt-1 text-[18px] font-semibold sm:text-[16px]">
                          Draw: {option.drawLabel}
                        </div>
                        <div className="mt-1 text-[20px] font-bold text-amber-300 sm:text-[17px]">
                          Amount: ₹{Number(option.totalPoints || 0)}
                        </div>
                        {option.isAdvanceDraw ? (
                          <div className="mt-2 inline-block rounded-md bg-[#1d4ed8] px-3 py-1.5 text-[14px] font-bold text-white sm:px-2.5 sm:py-1 sm:text-[13px]">
                            Advance Draw
                          </div>
                        ) : null}
                        {!singleOptionMode ? (
                            ...
                            <button
                              ...
                              onClick={() => {
                                setCancelBetDialog({
                                  ...
                                  description: `Cancel ${option.ticketId ? `ticket ${option.ticketId}` : `slot ${option.slotStartIso}`}?`,
                                  points: Number(option?.totalPoints || 0),
                                  ticketId: option.key,
                                  ticketBackendId: option?.ticketId || null,
                                  slotStartIso: option?.slotStartIso || '',
                                  ticketLabel: option?.ticketLabel || option?.slotStartIso || '',
                                });
                              }}
                              className="h-11 w-full rounded-lg border border-[#b91c1c] bg-[#991b1b] px-4 text-[18px] font-extrabold text-white hover:bg-[#b91c1c] sm:w-auto sm:text-[17px]"
                            >
                              Cancel This Bet
                            </button>
                          </div>
              ...
              <div
                className={`mt-5 grid grid-cols-1 gap-2 ${
                  cancelBetDialog.type === 'ticket-picker' && (Array.isArray(cancelBetDialog.options) ? cancelBetDialog.options.length : 0) === 1
                    ? 'sm:grid-cols-2'
                ...
                {cancelBetDialog.type === 'ticket-picker' && (Array.isArray(cancelBetDialog.options) ? cancelBetDialog.options.length : 0) === 1 ? (
                  <>
                    <button
                      ...
                      onClick={() => setCancelBetDialog(null)}
                      disabled={isCancelSubmitting}
                      className="h-[4rem] w-full rounded-lg border border-[#64748b] bg-[#1e293b] px-4 text-[22px] font-extrabold leading-none text-white disabled:opacity-60 sm:h-[4rem] sm:text-[22px]"
                    >
                      Close
                    </button>
                    <button
                      ...
                      disabled={isCancelSubmitting}
                      className="h-[4rem] w-full rounded-lg border border-[#b91c1c] bg-gradient-to-b from-[#ef4444] to-[#b91c1c] px-4 text-[22px] font-black leading-none text-white disabled:cursor-not-allowed disabled:opacity-60 sm:h-[4rem] sm:text-[22px]"
                    >
                      {isCancelSubmitting ? 'Cancelling...' : 'Cancel Bet Now'}
                    </button>
                  </>
                ) : cancelBetDialog.type === 'ticket-picker' ? (
                  <button
                    ...
                    onClick={() => setCancelBetDialog(null)}
                    disabled={isCancelSubmitting}
                    className="h-[4rem] w-full rounded-lg border border-[#64748b] bg-[#1e293b] px-4 text-[22px] font-extrabold leading-none text-white disabled:opacity-60 sm:h-[4rem] sm:flex-1 sm:text-[22px]"
                  >
                    Close
                  </button>
                ) : (
                  <>
                    ...
                    <button
                      ...
                      onClick={handleConfirmCancelBet}
                      disabled={isCancelSubmitting}
                      className="h-[4rem] w-full rounded-lg border border-[#b91c1c] bg-gradient-to-b from-[#ef4444] to-[#b91c1c] px-4 text-[22px] font-black leading-none text-white disabled:cursor-not-allowed disabled:opacity-60 sm:h-[4rem] sm:flex-1 sm:text-[22px]"
                    >
        ...
        {canUsePortal ? createPortal(
          <>
            <TicketListModal
              ...
              onView={(ticket) => {
                void handleOpenHistoryTicketDetails(ticket);
                setIsTicketListOpen(false);
              }}
            />
            <TicketListModal
