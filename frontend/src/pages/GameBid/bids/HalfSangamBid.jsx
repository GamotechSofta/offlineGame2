...
const HalfSangamBid = ({ market, title }) => {
    ...
    const submitBtnClass = (enabled) =>
        enabled
            ? 'w-full bg-gradient-to-r bg-[#1B3150] text-white font-bold py-3.5 min-h-[48px] rounded-lg shadow-md hover:bg-[#152842] transition-all active:scale-[0.98]'
    ...
    const inputCl = 'flex-1 min-w-0 bg-white border-2 border-gray-300 text-gray-800 placeholder-gray-400 rounded-full py-2.5 min-h-[40px] px-4 text-center text-sm focus:ring-2 focus:outline-none focus:ring-[#1B3150] focus:border-gray-300';
...
function HalfSangamFormSection({ flipped, setFlipped, bids, setBids, session, showWarning, inputCl, labelCl, sanitizeDigits, sanitizePoints, isValidAnyPana }) {
    ...
    return (
        <div className="p-4 space-y-3">
            ...
            <button type="button" onClick={handleAdd} className="w-full bg-gradient-to-r bg-[#1B3150] text-white font-bold py-3 min-h-[44px] rounded-lg shadow-md hover:bg-[#152842] transition-all active:scale-[0.98]">
                Add to List
            </button>
        </div>
    );
