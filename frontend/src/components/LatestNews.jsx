...
const LatestNews = () => {
  ...
  return (
    <div className="w-full bg-gradient-to-r from-gray-50 via-white to-gray-50 border-t-2 border-gray-300 shadow-md overflow-hidden relative max-w-full">
      ...
      <div className="flex items-center h-7 min-[375px]:h-8 md:h-9">
        ...
        <div className="shrink-0 bg-gradient-to-r from-gray-100 to-gray-50 border-r-2 border-gray-400 px-2 min-[375px]:px-3 sm:px-5 md:px-7 h-full flex items-center shadow-sm">
          <div className="flex items-center gap-1.5 min-[375px]:gap-2">
            <svg className="w-3.5 h-3.5 min-[375px]:w-4 min-[375px]:h-4 sm:w-5 sm:h-5 text-[#1B3150] shrink-0" fill="currentColor" viewBox="0 0 20 20">
              <path d="M2 6a2 2 0 012-2h6a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V6zM14.553 7.106A1 1 0 0014 8v4a1 1 0 00.553.894l2 1A1 1 0 0018 13V7a1 1 0 00-1.447-.894l-2 1z" />
            </svg>
            <span className="text-[#1B3150] font-bold text-[10px] min-[375px]:text-xs sm:text-sm md:text-base uppercase tracking-wider whitespace-nowrap">
              Latest News
            </span>
          </div>
        </div>
          ...
          <div className="flex animate-scroll-news h-full items-center">
            {duplicatedItems.map((item, index) => (
              ...
              >
                ...
                <div className="flex items-center justify-center w-6 h-6 min-[375px]:w-7 min-[375px]:h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-full bg-gradient-to-br from-gray-100 to-gray-50 border-2 border-gray-300 shadow-sm shrink-0">
                  <span className="text-sm min-[375px]:text-base sm:text-lg md:text-xl leading-none">{item.icon}</span>
                </div>
                
                {/* Text content */}
                <div className="flex items-center gap-2 sm:gap-3">
                  <span className="text-gray-800 text-[10px] min-[375px]:text-xs sm:text-sm md:text-base font-semibold whitespace-nowrap group-hover:text-[#1B3150] transition-colors truncate max-w-[200px] min-[375px]:max-w-none">
                    {item.text}
                  </span>
                  {item.amount && (
                    <span className="text-[#1B3150] font-bold text-[10px] min-[375px]:text-xs sm:text-sm md:text-base whitespace-nowrap shrink-0">
                      {item.amount}
                    </span>
                  )}
                </div>
                ...
                {/* Separator dot */}
                <div className="w-1.5 h-1.5 bg-[#1B3150] rounded-full shadow-sm"></div>
              </div>
            ))}
          </div>
        </div>
