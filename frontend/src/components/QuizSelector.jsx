    ...
    <div className="bg-[#efefef] text-black border-b border-[#5f5f5f]">
      {QUIZ_GROUPS.map((group) => (
        <div key={group.setName} className="grid grid-cols-[86px_1fr_136px]">
          <div className="bg-[#e95757] text-white text-center h-11 leading-[44px] text-[14px] font-semibold border border-[#d1d1d1]">{group.setName}</div>
            ...
            {Array.from({ length: group.end - group.start + 1 }, (_, idx) => group.start + idx).map((quizNo) => {
              ...
              return (
                <button
                  ...
                  className={`h-11 border text-[14px] font-medium rounded-none ${getSetColors(group.setName, isActive)}`}
                >
                  {quizLabel(quizNo)}
                </button>
              );
          ...
          <div className="border border-[#8f8f8f] bg-[#efefef] flex items-center justify-stretch text-[14px] h-11 p-0">
            {group.setName === 'Set A' && (
              <label
