...
const PlaceBetForPlayer = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                setSuccess(`Bet placed successfully! Player balance: ₹${Number(data.data?.newBalance ?? 0).toLocaleString('en-IN')}`);
                setBets([{ betNumber: '', amount: '' }]);
                if (data.data?.newBalance != null) {
                    setPlayers((prev) =>
                        prev.map((p) =>
                            p._id === selectedPlayerId
                                ? { ...p, walletBalance: Number(data.data.newBalance) }
                                : p,
                        ),
                    );
                }
        ...
        <Layout title={gameMeta.label}>
            <div className="min-w-0 max-w-full">
                ...
                <button
                    ...
                    className="text-gray-400 hover:text-sb-primary text-sm inline-flex items-center gap-1 mb-4"
                >
                    ...
                    <>
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 mb-5">
                            <div className="flex items-center gap-3 flex-wrap">
                                <FaDice className="text-sb-primary w-6 h-6" />
                                <div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
                                <div className="flex items-center justify-between mb-3">
                                    <h2 className="text-base font-semibold text-sb-primary flex items-center gap-2">
                                ...
                                {selectedPlayer && !showPlayerList && (
                                    <div className="flex items-center gap-3 p-3 bg-sb-primary/5 border border-sb-primary/20 rounded-lg">
                                        <div className="w-10 h-10 rounded-full bg-orange-500/20 flex items-center justify-center">
                                            <FaUser className="w-4 h-4 text-sb-primary" />
                                        </div>
                                    ...
                                    <>
                                        {loadingPlayers ? (
                                            ...
                                            <p className="text-gray-400 text-sm">
                                                No players found.{' '}
                                                <button type="button" onClick={() => navigate('/add-user')} className="text-sb-primary hover:underline">
                                                    Add a player first
                                                </button>
                                            </p>
                                        ) : (
                                            <>
                                                <div className="relative mb-3">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full pl-9 pr-4 py-2.5 bg-gray-100/80 border border-gray-200 rounded-lg text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-sb-primary/50 focus:border-sb-primary text-sm"
                                                    />
                                                </div>
                                                        ...
                                                        <button
                                                            ...
                                                            className={`text-left px-3 py-2.5 rounded-lg border transition-all text-sm ${
                                                                selectedPlayerId === p._id
                                                                    ? 'bg-sb-primary/20 border-sb-primary text-sb-primary'
                                                                    : 'bg-gray-50 border-gray-200 text-gray-600 hover:border-gray-300 hover:bg-gray-100'
                                                            }`}
                                                        >
                                                            <div className="flex items-center justify-between">
                                                                <span className="font-medium truncate">{p.username}</span>
                                                                {selectedPlayerId === p._id && <FaCheck className="w-3 h-3 text-sb-primary shrink-0 ml-1" />}
                                                            </div>
                            ...
                            {/* Session */}
                            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
                                <label className="block text-sm font-semibold text-sb-primary mb-3">Session</label>
                                    ...
                                    {['open', 'close'].map((s) => (
                                        <button
                                            ...
                                            className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${
                                                session === s
                                                    ? 'bg-sb-primary text-white shadow-lg shadow-sb-primary/20'
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
                                <div className="flex items-center justify-between mb-3">
                                    <h2 className="text-base font-semibold text-sb-primary">
                                        ...
                                        <div key={idx} className="flex gap-2 items-center">
                                            <div className="flex-1">
                                                <input
                                                    ...
                                                    className={`w-full px-3 py-2.5 bg-gray-100 border rounded-lg text-gray-800 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-sb-primary/50 font-mono ${
                                            ...
                                            <div className="w-28">
                                                <input
                                                    ...
                                                    placeholder="Amount ₹"
                                                    className="w-full px-3 py-2.5 bg-gray-100 border border-gray-200 rounded-lg text-gray-800 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-sb-primary/50"
                                                />
                                            </div>
                                            {bets.length > 1 && (
                                                <button
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
                                <div className="grid grid-cols-3 gap-3 text-sm mb-4">
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-3 text-center">
                                        ...
                                        <p className="text-sb-primary font-mono font-bold text-lg">₹{totalAmount.toLocaleString('en-IN')}</p>
                                    </div>
                                    <div className="bg-gray-50 rounded-lg p-3 text-center">
                                ...
                                <button
                                    ...
                                    className="w-full bg-gradient-to-r bg-sb-primary hover:bg-sb-primary-dark text-gray-800 font-bold py-3.5 px-4 rounded-xl transition-all disabled:opacity-50 shadow-lg shadow-sb-primary/20 text-base"
                                >
                                    {placing ? (
