    ...
    return (
        <Layout title={t('wallet')}>
            ...
            <div className="flex gap-4 mb-4 sm:mb-6 border-b border-gray-200">
                <button onClick={() => setActiveTab('wallets')} className={`pb-4 px-4 font-semibold transition-colors ${activeTab === 'wallets' ? 'text-sb-primary border-b-2 border-sb-primary' : 'text-gray-400 hover:text-gray-600'}`}>{t('playerWallets')}</button>
                <button onClick={() => setActiveTab('transactions')} className={`pb-4 px-4 font-semibold transition-colors ${activeTab === 'transactions' ? 'text-sb-primary border-b-2 border-sb-primary' : 'text-gray-400 hover:text-gray-600'}`}>{t('transactions')}</button>
            </div>
            {loading ? (
                        ...
                        <tbody className="divide-y divide-gray-700">
                            {wallets.length === 0 ? <tr><td colSpan="2" className="px-6 py-4 text-center text-gray-400">No wallets found</td></tr> : wallets.map((w) => (
                                <tr key={w._id} className="hover:bg-gray-100">
                                    ...
                                    <td className="px-6 py-4 text-sm font-semibold text-sb-primary">₹{w.balance}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
