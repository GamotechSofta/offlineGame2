...
const HalfSangamABid = ({ title }) => {
    ...
    const handleSubmitBet = async () => {
        ...
        const payload = bids.map((b) => ({
            betType: 'half-sangam', betNumber: String(b?.number ?? '').trim(), amount: Number(b?.points) || 0,
    ...
    return (
        <BookieBidLayout title={title} bidsCount={bids.length} totalPoints={totalPoints} showDateSession={true}
            ...
            sessionOptionsOverride={['OPEN']} lockSessionSelect hideSessionSelectCaret hideFooter contentPaddingClass="pb-6">
                ...
                <div className="md:grid md:grid-cols-2 md:gap-12 md:items-start">
                    ...
                    <div className="mt-10 md:mt-0">
                        <div className="grid grid-cols-[1.4fr_0.7fr_0.6fr] gap-2 sm:gap-3 text-center text-[#d4af37] font-bold text-xs sm:text-sm mb-2 px-2">
                            <div className="truncate">Sangam</div><div className="truncate">Amount</div><div className="truncate">Delete</div>
                        </div>
                        <div className="h-px bg-white/10 w-full mb-2" />
                        {bids.length > 0 && (
                            <div className="space-y-2">
                                {bids.map((b) => (
                                    <div key={b.id} className="grid grid-cols-[1.4fr_0.7fr_0.6fr] gap-2 sm:gap-3 text-center items-center py-2.5 px-3 bg-[#202124] rounded-lg border border-white/10 text-sm">
