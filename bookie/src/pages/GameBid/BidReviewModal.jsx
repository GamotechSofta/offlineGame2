      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          td.points { font-weight: 700; color: #6d28d9; }
          td.session { text-transform: uppercase; color: #666; font-size: 10px; }
          .summary { margin-top: 14px; border-top: 2px solid #6d28d9; padding-top: 10px; }
          ...
          .summary-row .value.orange { color: #6d28d9; }
      ...
      <div className="relative w-full max-w-md sm:max-w-lg">
        {stage === 'success' && receiptData ? (
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200 max-h-[min(92svh,780px)] sm:max-h-[calc(100vh-48px)] flex flex-col">
                    ...
                    {Object.entries(receiptData.grouped).map(([label, items]) => (
                      <div key={label} className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                        <div className="px-3 py-2 bg-gray-100 border-b border-gray-200 flex items-center justify-between">
                          <span className="text-sb-primary font-semibold text-[10px] uppercase tracking-wide">{label} <span className="text-gray-400">({items.length})</span></span>
                          <span className="text-sb-primary text-[11px] font-bold">₹{items.reduce((s, r) => s + Number(r.points || 0), 0).toLocaleString('en-IN')}</span>
                        </div>
                          ...
                          {items.map((r) => (
                            <div key={r.id} className="px-2 py-1.5 border-b border-gray-100 last:border-b-0">
                              <div className="flex items-center justify-between gap-2 text-xs">
                                ...
                                <span className="text-sb-primary font-bold shrink-0">₹{r.points}</span>
                  ...
                  <div className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                    <div className="px-3 py-2 bg-gray-100 border-b border-gray-200">
                      <div className="grid grid-cols-2 divide-x divide-gray-300">
                        <div className="text-center text-[10px] font-semibold text-sb-primary uppercase">
                ...
                <div className="rounded-xl border border-gray-200 overflow-hidden">
                  <div className="grid grid-cols-2 divide-x divide-gray-200">
                    ...
                    <div className="p-3 text-center border-b border-gray-200">
                      ...
                      <p className="text-sb-primary font-bold text-lg">₹{receiptData.totalAmount.toLocaleString('en-IN')}</p>
                    </div>
            ...
            <div className="px-4 py-3 border-t border-gray-200 bg-white shrink-0 flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-sb-primary text-white font-bold py-3 rounded-xl shadow-md active:scale-[0.99] transition-transform hover:bg-sb-primary-dark"
              >
                Done
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200 max-h-[min(90svh,720px)] sm:max-h-[calc(100vh-48px)] flex flex-col">
            <div className="bg-sb-primary text-white px-3 sm:px-4 py-2.5 text-center text-sm sm:text-lg font-semibold shrink-0 border-b border-gray-200">
              {formatDateTitle(marketTitle, dateText)}
            </div>

            {playerName && (
              <div className="px-3 sm:px-4 pt-2 shrink-0">
                <div className="bg-sb-primary/5 border border-sb-primary/20 rounded-lg px-3 py-2 text-sm text-sb-primary font-medium text-center">
                  Placing bet for: <span className="font-bold text-sb-primary-dark">{playerName}</span>
                </div>
              </div>
            )}
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex-1 overflow-y-auto overscroll-contain px-3 sm:px-4 pt-3 sm:pt-4 min-h-0">
                {showGameType && groupedRows ? (
                    ...
                    {Object.entries(groupedRows).map(([gameLabel, gameRows]) => (
                      <div key={gameLabel}>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-sb-primary font-semibold text-xs uppercase tracking-wide">{gameLabel}</span>
                          <span className="text-gray-500 text-xs">({gameRows.length})</span>
                          <span className="text-sb-primary text-xs font-bold ml-auto">
                          ...
                          {gameRows.map((r) => (
                            <div key={r.id} className="bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
                              <div className="grid grid-cols-3 text-center text-gray-800 text-[12px] sm:text-sm">
                                ...
                                <div className="font-bold text-sb-primary truncate">{r.points}</div>
                  ...
                  /* Flat list (original) */
                  <>
                    <div className="grid grid-cols-3 text-center font-semibold text-sb-primary text-[11px] sm:text-base">
                      ...
                      {rows.map((r) => (
                        <div key={r.id} className="bg-gray-50 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200">
                          <div className="grid grid-cols-3 text-center text-gray-800 font-semibold text-[12px] sm:text-base">
                            ...
                            <div className="truncate text-sb-primary">{r.points}</div>
                ...
                <div className="rounded-2xl overflow-hidden border border-gray-200">
                  <div className="grid grid-cols-2">
                    ...
                    <div className="p-3 sm:p-4 text-center border-b border-gray-200">
                      ...
                      <div className="text-gray-800 font-bold text-base sm:text-lg text-sb-primary leading-tight">{amount}</div>
                    </div>
              ...
              {insufficientBalance && (
                <div className="mx-3 sm:mx-4 mt-2 p-3 rounded-xl bg-sb-primary/5 border border-sb-primary/20 text-sb-primary text-sm shrink-0">
            ...
            <div className="px-3 sm:px-4 py-3 sm:py-4 grid grid-cols-2 gap-3 sm:gap-4 bg-white shrink-0 border-t border-gray-200">
              <button
                ...
                onClick={handleClose}
                disabled={submitting}
                className="bg-gray-100 border border-gray-200 text-gray-800 font-bold py-3 rounded-xl sm:rounded-2xl shadow-md active:scale-[0.99] transition-transform hover:border-sb-primary/40 disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                className="bg-sb-primary text-white font-bold py-3 rounded-xl sm:rounded-2xl shadow-md active:scale-[0.99] transition-transform hover:bg-sb-primary-dark disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {submitting ? (
                  <>
