...
// Double Pana validation (from frontend panaRules / DoublePanaBid)
...
const getAllValidDoublePana = () => {
  ...
  for (let i = 100; i <= 999; i++) {
    const str = String(i);
    if (validateDoublePana(str)) valid.push(str);
  }
  return valid;
};

const VALID_DOUBLE_PANAS = getAllValidDoublePana();
const VALID_SET = new Set(VALID_DOUBLE_PANAS);

const findPanaBySum = (targetNum) => {
  const matches = [];
  for (const pana of VALID_DOUBLE_PANAS) {
...
export default function DoublePanaBid({ market, title }) {
  ...
  const handleAddToList = () => {
    ...
    if (!validateDoublePana(pana)) {
      showWarning('Invalid Double Pana. Use 3 digits with two same (e.g. 112, 220).');
      return;
    }
    if (!pts || pts <= 0) {
  ...
  const handleSumClick = (num) => {
    ...
    if (matches.length === 0) {
      showWarning(`No double pana with sum ${num}.`);
      return;
    }
    ...
    showWarning(`Added ${matches.length} double pana with sum ${num}`);
  };
