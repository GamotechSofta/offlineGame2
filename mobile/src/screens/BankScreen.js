...
export default function BankScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const errMsg = !t?.success && t?.message ? t.message : !b?.success && b?.message ? b.message : '';
        ...
        {!isLoggedIn ? (
          ...
          <View style={{ padding: 16 }}>
            <View style={[styles.card, { padding: 24, marginBottom: 16 }]}>
              ...
              <SkeletonBox width={100} height={28} style={{ marginBottom: 16 }} />
            ...
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <SkeletonRow key={i} hasIcon={false} />
            ))}
          </View>
        ) : error ? (
