            ...
            <div className="w-full min-w-0 px-3 sm:px-4 md:px-6 pb-6 sm:pb-8">
                ...
                {activeTab === 'starline' && (
                    <div className="flex flex-col xl:flex-row gap-4 sm:gap-6">
                        <div className="flex-1 min-w-0 w-full">
                            {loading ? (
                                <div className="text-center py-8 sm:py-12 text-gray-400 text-xs sm:text-sm rounded-xl border border-gray-200 bg-white">Loading Starline slots...</div>
                            ) : starlineMarkets.length === 0 ? (
                                <div className="rounded-2xl border border-orange-200 bg-white p-6 sm:p-8 text-center">
                                    <p className="text-gray-400 text-sm mb-4">No Starline slots yet. Add markets and slots from Markets → Starline Market.</p>
                                    <Link to="/markets" state={{ marketType: 'starline' }} className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-orange-500 hover:bg-amber-400 text-gray-800 font-semibold text-sm">
                                        <FaStar className="w-4 h-4" /> Go to Starline Market
                                    </Link>
                                </div>
                            ) : (
                                <div className="overflow-x-auto rounded-lg sm:rounded-xl border border-gray-200 bg-white shadow-lg">
                                    <table className="w-full border-collapse text-[11px] sm:text-xs md:text-sm min-w-[320px]">
                                        <thead>
                                            <tr className="border-b border-gray-200">
                                                <th className="text-left py-2 sm:py-3 px-3 font-semibold text-orange-500 bg-white">Slot</th>
                                                <th className="text-left py-2 sm:py-3 px-3 font-semibold text-orange-500 bg-white border-l border-gray-200">Closes</th>
                                                <th className="text-left py-2 sm:py-3 px-3 font-semibold text-orange-500 bg-white border-l border-gray-200">Result</th>
                                                <th className="text-left py-2 sm:py-3 px-3 font-semibold text-orange-500 bg-white border-l border-gray-200">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {starlineMarkets.map((m) => {
                                                ...
                                                return (
                                                    <tr key={m._id} className={`border-b border-gray-200 hover:bg-gray-50 ${isPending ? 'bg-orange-500/5' : ''}`}>
                                                        <td className="py-2 sm:py-3 px-3 font-medium text-gray-800 truncate max-w-[180px]">
                                                            {isPending && <FaExclamationTriangle className="inline w-3.5 h-3.5 text-orange-500 mr-1.5 align-middle" />}
                                                            {m.marketName}
                                                        </td>
                                                        <td className="py-2 sm:py-3 px-3 text-gray-600 border-l border-gray-200 whitespace-nowrap">{formatTime(m.closingTime)}</td>
                                                        <td className="py-2 sm:py-3 px-3 border-l border-gray-200 font-mono text-orange-500">{resultDisplay}</td>
                                                        <td className="py-2 sm:py-3 px-3 border-l border-gray-200">
                                                            <button
                                                                type="button"
                                                                onClick={() => openPanelForEdit(m)}
                                                                className="px-2 sm:px-3 py-1.5 sm:py-2 bg-orange-500 hover:bg-orange-600 text-gray-800 font-semibold rounded-lg text-xs sm:text-sm"
                                                            >
                        ...
                        {selectedMarket?.marketType === 'startline' && (
                            <div className={`w-full bg-white rounded-xl border border-gray-200 shadow-xl p-4 sm:p-5 md:p-6 ${isDirectEditMode ? 'max-w-lg mx-auto' : 'xl:w-[380px] xl:max-w-[400px] xl:shrink-0'}`}>
                                <h2 className="text-lg sm:text-xl font-bold text-orange-500 mb-1 border-b border-gray-200 pb-2 truncate" title={selectedMarket.marketName}>{selectedMarket.marketName}</h2>
                                {getMarketId() && (
                                ...
                                {(selectedMarket.openingNumber && /^\d{3}$/.test(String(selectedMarket.openingNumber))) && (
                                    <button
                                        ...
                                        className="mb-3 sm:mb-4 w-full px-4 py-2.5 sm:py-3 bg-red-900/80 hover:bg-red-800 text-red-100 font-semibold rounded-lg border border-red-200 disabled:opacity-50 transition-colors text-sm sm:text-base"
                                    >
                                ...
                                <button
                                    type="button"
                                    onClick={closePanel}
                                    className="w-full px-4 py-2.5 sm:py-3 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold rounded-lg border border-gray-200 transition-colors text-sm sm:text-base"
                                >
                                    Close
                                </button>
                            </div>
                ...
                {activeTab === 'king' && (
                    <div className="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 text-center">
                        ...
                        <h2 className="text-lg font-bold text-gray-800 mb-2">King Bazaar Result</h2>
