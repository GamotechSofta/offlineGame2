...
const OldSlotsSection = ({
    ...
    setPlayerBetsAggregateAllSlots,
                    ...
                    <div className="flex flex-wrap items-end gap-4">
                        ...
                        <div ref={timeDropdownRef} className="min-w-[220px] flex-1 relative">
                            ...
                            <button
                                ...
                                disabled={!historySlots.length}
                            >
                                <span>{playerBetsAggregateAllSlots ? 'All slots (full day)' : selectedSlotMeta?.drawLabelEnd || 'No slots available for selected date'}</span>
                                <FaChevronDown className={`text-xs text-gray-500 transition-transform ${isTimeDropdownOpen ? 'rotate-180' : ''}`} />
                            </button>
                            {isTimeDropdownOpen && historySlots.length ? (
                                <div className="absolute z-20 mt-2 w-full rounded-xl border border-gray-200 bg-white shadow-lg overflow-hidden">
                                    <div className="max-h-64 overflow-y-auto py-1">
                                        <button
                                            ...
                                            className={`w-full px-3 py-2 text-left text-sm transition ${
                                                playerBetsAggregateAllSlots ? 'bg-orange-50 text-orange-600 font-semibold' : 'text-gray-700 hover:bg-gray-50'
                                            }`}
                                        >
                                            All slots (full day)
                                        </button>
                                        {historySlots.map((slot) => {
                                            const active = !playerBetsAggregateAllSlots && slot.slotStartIso === selectedSlot;
                                            return (
                                                <button
                        ...
                        <div className="text-sm text-gray-500">
                            {playerBetsAggregateAllSlots ? (
                                ...
                                <>
                                    Showing playing players for <span className="font-semibold text-gray-700">{selectedSlotMeta.drawLabelEnd}</span> only
                                </>
                            ) : null}
                        </div>
                    </div>
