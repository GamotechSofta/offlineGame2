...
import { buildMenuForAdmin, getSidebarTitle, getSidebarAbbrev } from '../config/adminMenuItems';
...
const Sidebar = ({
    ...
    const allowedTabsKey = JSON.stringify(admin?.allowedTabs || []);
    const menuItems = useMemo(() => buildMenuForAdmin(admin), [admin?.role, allowedTabsKey]);

    const sidebarTitle = getSidebarTitle(adminRole);

    const sidebarAbbrev = getSidebarAbbrev(adminRole);
