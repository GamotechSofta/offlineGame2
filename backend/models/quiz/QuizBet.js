...
const quizBetSchema = new mongoose.Schema(
  {
    ...
    /** One BUY action = one ticketId across all inserted rows. */
...
quizBetSchema.index(
  { gameMode: 1, betOwnerKey: 1, ticketId: 1, quizId: 1, slotStartIso: 1, number: 1, betMode: 1 },
  {
    unique: true,
...
quizBetSchema.index({ gameMode: 1, slotStartIso: 1, userId: 1, ticketId: 1, createdAt: -1 });
quizBetSchema.index({ gameMode: 1, slotStartIso: 1, createdAt: -1 });
